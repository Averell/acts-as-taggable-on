  ...
  module Collection
    ...
    module ClassMethods
      ...
      def all_tag_counts(options = {})
        ...
        taggable_conditions  = ""
        if ActsAsTaggableOn::Tagging.taggable_context_crc32? && options[:id] && options[:on]
          taggable_conditions << sanitize_sql(["#{ActsAsTaggableOn::Tagging.table_name}.taggable_context_crc32 = ? AND ", ActsAsTaggableOn::Tagging.taggable_context_crc32(base_class.name, options[:id], options[:on])])
        end
